...
export const generateBillPdf = async (bill, user, org, res) => {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(9).font('Helvetica').fillColor('#666').text(`#${bill._id.toString().slice(-8).toUpperCase()}`, 400, 62, { align: 'right' });
    ...
    bill.calculationBreakdown.forEach((row, i) => {
    ...
    doc.fontSize(13).text(`₹${(bill.totalInPaise / 100).toLocaleString('en-IN')}`, 460, rowY + 12, { width: 80, align: 'right' });
